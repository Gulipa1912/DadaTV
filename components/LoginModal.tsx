...
const LoginModal = () => {
  ...
  const handleLogin = async () => {
    ...
    if (!password || (!isLocalStorage && !username)) {
      Toast.show({ type: "error", text1: "請輸入用戶名及密碼" });
      return;
    }
    setIsLoading(true);
    try {
      ...
      Alert.alert(
        "免責聲明",
        "本應用程式僅提供影視資訊搜尋服務，所有內容均來自第三方網站。本站不儲存任何影片資源，不對任何內容的準確性、合法性、完整性負責。",
        [{ text: "確定" }]
      );
    } catch (error) {
      Toast.show({
        type: "error",
        text1: "登录失败",
        text2: error instanceof Error ? error.message : "用户名或密碼錯誤",
      });
    } finally {
      setIsLoading(false);
    }
    ...
    >
      <View style={styles.overlay}>
        <ThemedView style={styles.container}>
          ...
          <ThemedText style={styles.subtitle}>伺服器需要驗證您的身分</ThemedText>
