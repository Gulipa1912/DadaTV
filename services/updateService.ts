    ...
    while (retries < maxRetries) {
      try {
        ...
        if (retries === maxRetries) {
          Toast.show({ type: "error", text1: "檢查更新失敗", text2: "無法獲取版本訊息，請檢察網路連線" });
          throw error;
        }
  ...
  async downloadApk(url: string, onProgress?: (progress: number) => void): Promise<string> {
    ...
    while (retries < maxRetries) {
      try {
        ...
        if (retries === maxRetries) {
          Toast.show({ type: "error", text1: "下載失敗", text2: "APK下載失敗，請檢查網路連線" });
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes('No app found')) {
          Toast.show({ type: "error", text1: "安裝失敗", text2: "未找到可安装APK的应用，請確保允許安裝未知來源的應用 });
          throw new Error('未找到可安裝APK的應用，請確保允許安裝未知來源的應用');
        } else if (error.message.includes('permission')) {
          Toast.show({ type: "error", text1: "安裝失敗", text2: "沒有安裝權限，請在設定中允許此應用程式安裝未知來源的應用" });
          throw new Error('沒有安裝權限，請允許此應用程式安裝未知來源的應用');
        } else {
          Toast.show({ type: "error", text1: "安裝失敗", text2: "APK安裝過程中出現錯誤" });
        }
      } else {
        Toast.show({ type: "error", text1: "安裝失敗", text2: "APK安裝過程中出現未知錯誤" });
      }
